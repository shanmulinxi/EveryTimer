...
module.exports = class BodyCenter extends Control {
  ...
  insertBodyData(req, res) {
    ...
    if (!reqData || !reqUser) {
      this.failReturn(res, ErrorCode.BodyCenter_Insert_NullReqData,"??没有请求的数据呀")
      return
    }
    // 通过basebody类构建数据对象
    ...
    Base_Body
      ...
      .catch(err => {
        console.log(err)
        this.failReturn(res, ErrorCode.BodyCenter_Insert_InsertSQL,"抱歉出错啦!")
        return
      })
  }
  ...
  deleteBodyData(req, res){
    ...
    const reqUser = req.body.user || null
    if (!reqData || !reqUser) {
      this.failReturn(res, ErrorCode.BodyCenter_Delete_NullReqData,"??没有请求的数据呀")
      return
    }

    //获取数据
    Base_Body.getDataFormId(reqData['id']).then(searchR=>{
      if (searchR.length == 0) {
      ...
      Base_Body.updateBody(operateData,['isDelete','editTime']).then(result=>{
        this.successReturn(res, {})
      }).catch(err=>{
        console.log(err)
  ...
  * 修改body数据方法
  ...
  updateBodyData(req, res){
    ...
    Base_Body.getDataFormId(reqData['id']).then(searchR=>{
      ...
      if (verifymsg != true ) {
        this.failReturn(res,  ErrorCode.BodyCenter_UpdateBody_VerifyAllData,verifymsg)
        return
      }
      // 调整修改时间
      operateData['editTime'] = Moment().format('YYYY-MM-DD HH:mm:ss')
      Base_Body.updateBody(operateData,[...sqlList,'editTime']).then(result=>{
        this.successReturn(res, {})
      }).catch(err=>{
        console.log(err)
  ...
  * 查找body数据方法
  ...
  searchBodyData(req, res){
    ...
    const reqUser = req.body.user || null
    if (!reqData || !reqUser) {
      this.failReturn(res, ErrorCode.BodyCenter_SearchBody_NullReqData,"??没有请求的数据呀")
      return
    }
